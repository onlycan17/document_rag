...
class QueryEngine:
    ...
    def query(self, question: str) -> Dict[str, Any]:
        ...
        try:
            ...
            if hasattr(self.llm_manager, 'current_provider') and self.llm_manager.current_provider == "local":
    ...
    def extract_keywords(self, query: str) -> List[str]:
        """쿼리에서 핵심 키워드 추출"""
        # 간단한 키워드 추출 (불용어 제거)
        stop_words = {'은', '는', '이', '가', '을', '를', '에', '에서', '로', '으로', '와', '과', '의', '에게', '한테', '께',
                     '뭐', '무엇', '어떤', '어떻게', '왜', '언제', '어디', '누구', '얼마나', '몇', '어느',
                     '알려줘', '알려주세요', '설명해줘', '설명해주세요', '말해줘', '말해주세요', '가르쳐줘', '가르쳐주세요'}
