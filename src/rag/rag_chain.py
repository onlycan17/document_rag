...
from src.constants import (
    ...
    REQUEST_TIMEOUT, LOCAL_MODEL_TIMEOUT, MAX_RETRIES, RETRY_DELAY
)
...
class RAGChain:
    ...
    def __init__(self, provider: Optional[str] = None, model: Optional[str] = None, vector_db: Optional[VectorDatabase] = None):
        ...
        self.document_processor = DocumentProcessor(
            max_context_length_func=self._get_max_context_length_for_model
        )
    ...
    def _initialize_components(self, provider: Optional[str], model: Optional[str]):
        ...
        # 로컬 GGUF 세부 동작 제어 플래그 설정
        self.is_qwen_gguf = getattr(self.llm_manager, 'is_qwen_gguf', False)
        self.is_midm_gguf = getattr(self.llm_manager, 'is_midm_gguf', False)
    ...
    def _get_max_context_length_for_model(self) -> int:
        ...
        return self.llm_manager.get_max_context_length_for_model(
