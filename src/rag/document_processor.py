...
class DocumentProcessor:
    ...
    def rerank_documents(self, documents: List[tuple]) -> List[tuple]:
        ...
        for doc, original_score in documents:
            ...
            final_score = relevance_score * 0.7 + quality_score * 0.1 + length_score * 0.1 + metadata_score * 0.1
    ...
    def calculate_document_quality(self, doc: Document) -> float:
        ...
        # 한국어 문장 완성도 체크
        korean_sentences = len(
            [s for s in content.split(".") if any("\u3131" <= c <= "\u3163" or "\uac00" <= c <= "\ud7a3" for c in s)]
        )
