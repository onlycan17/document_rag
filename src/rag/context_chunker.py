...
class ContextChunk:
    ...
    def get_metadata(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "file_sources": list(set(doc.metadata.get("file_name", "Unknown") for doc, _ in self.documents)),
        }
...
class ContextChunker:
    ...
    def split_documents(
        self, documents: List[Tuple[Document, float]], query: str = "", strategy: str = "relevance"
    ) -> List[ContextChunk]:
        """
        문서들을 여러 청크로 분할
        ...
        logger.info(
            f"컨텍스트 분할 시작: {len(documents)}개 문서, {total_length:,}자 → 최대 {self.max_chunk_size:,}자 청크로 분할"
        )
    ...
    def _split_by_relevance(self, documents: List[Tuple[Document, float]], query: str) -> List[ContextChunk]:
        """관련도 기반 분할"""
        # 관련도 순으로 정렬
    ...
    def _split_by_topic(self, documents: List[Tuple[Document, float]], query: str) -> List[ContextChunk]:
        """주제별 분할 (클러스터링 기반)"""
        if len(documents) < 3:
        ...
        try:
            ...
            for cluster_id, cluster_docs in clusters.items():
                ...
                if cluster_length > self.max_chunk_size:
                    ...
                    # 주제 키워드 추출
                    [idx for _, _, idx in cluster_docs]
    ...
    def _split_hybrid(self, documents: List[Tuple[Document, float]], query: str) -> List[ContextChunk]:
        """하이브리드 분할 (주제별 + 관련도 혼합)"""
        # 먼저 주제별로 분할 시도
    ...
    def optimize_chunk_order(self, chunks: List[ContextChunk], query: str) -> List[ContextChunk]:
        """청크 순서 최적화 (관련도 기반)"""
        if not chunks:
            return chunks
