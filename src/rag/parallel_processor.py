...
from .summarizer import HierarchicalSummarizer, DocumentSummary
...
class ParallelRAGProcessor:
    ...
    async def process_chunks_parallel(self, 
                                    ...
                                    query: str,
                                    rag_chain_func: Callable) -> Dict[str, Any]:
        """
        병렬로 청크들을 처리
        ...
        processing_results = await self._process_chunks_async(
            prioritized_chunks, query, rag_chain_func
        )
    ...
    def process_chunks_sequential(self,
                                chunks: List[ContextChunk],
                                query: str,
                                rag_chain_func: Callable,
                                early_stop_threshold: float = 0.8) -> Dict[str, Any]:
        """
        ...
        for chunk in prioritized_chunks:
            try:
                ...
                # 조기 종료 조건 확인
                if (chunk_result.success and 
                    self._should_early_stop(chunk_result, early_stop_threshold)):
    ...
    async def _process_chunks_async(self,
                                  chunks: List[ContextChunk],
                                  query: str,
                                  rag_chain_func: Callable) -> List[ProcessingResult]:
        """비동기로 청크들을 처리"""
        ...
        with concurrent.futures.ThreadPoolExecutor(max_workers=self.max_workers) as executor:
            ...
            try:
                results = await asyncio.wait_for(
                    ...
                    timeout=self.chunk_timeout * len(chunks)
                )
    ...
    async def _advanced_merge_strategy(self, 
                                     results: List[ProcessingResult], 
                                     query: str) -> Dict[str, Any]:
        """고급 병합 전략"""
        # 결과들을 관련도 순으로 정렬
        sorted_results = sorted(results, 
                              key=lambda r: r.metadata.get('relevance_score', 0), 
                              reverse=True)
        ...
        if self.summarizer:
            try:
                ...
                # 계층적 요약 실행
                summary_result = self.summarizer.create_multi_level_summary(
                    result_documents, query, levels=2
                )
    ...
    def _simple_merge_strategy(self, 
                             results: List[ProcessingResult], 
                             query: str) -> str:
        """단순 병합 전략"""
        if not results:
    ...
    def _merge_sequential_results(self, 
                                results: List[ProcessingResult], 
                                query: str) -> str:
        """순차 처리 결과 병합"""
        successful_results = [r for r in results if r.success and r.result]
    ...
    def _should_early_stop(self, 
        ...
        return (relevance_score >= threshold and 
