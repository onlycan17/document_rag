...
class ContentProcessor:
    ...
    def process_documents(self, documents: List[Document], 
        ...
        try:
            ...
            if self.enable_parallel_processing and len(documents) > 1:
                processed_docs = self._process_documents_parallel(
                    documents, file_path, progress_callback
                )
            else:
                processed_docs = self._process_documents_sequential(
                    documents, file_path, progress_callback
                )
    ...
    def _process_documents_parallel(self, documents: List[Document], 
                                   file_path: Optional[str] = None,
                                   progress_callback: Optional[Callable] = None) -> List[Document]:
        """병렬 문서 처리"""
        try:
    ...
    def _process_documents_sequential(self, documents: List[Document], 
                                    file_path: Optional[str] = None,
                                    progress_callback: Optional[Callable] = None) -> List[Document]:
        """순차 문서 처리"""
        try:
    ...
    def _process_single_document(self, document: Document, 
                               file_path: Optional[str] = None,
                               doc_index: int = 0,
                               total_docs: int = 1) -> List[Document]:
        """단일 문서 처리"""
        try:
            # 처리 컨텍스트 생성
            context = {
    ...
    def _process_multimodal_content(self, context: Dict[str, Any]) -> Dict[str, Any]:
        ...
        try:
            ...
            if (file_path and 
                ...
                'openrouter' in self.multimodal_services):
                ...
                if image_analysis:
                    ...
                    # 이미지 정보를 텍스트에 통합
                    document = context['document']
                    enhanced_content = self._integrate_image_analysis(
    ...
    def _process_with_agents(self, context: Dict[str, Any]) -> Dict[str, Any]:
        ...
        try:
            # PDF 변환 에이전트 사용
            if ('pdf_converter' in self.agents and 
                context.get('file_path') and 
                Path(context['file_path']).suffix.lower() == '.pdf'):
