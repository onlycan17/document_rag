        ...
        if self.use_intelligent_image_extraction:
            try:
                ...
                try:
                    ...
                    extraction_results = service.extract_and_analyze_pdf_images(file_path, str(output_dir))
                ...
                if extraction_results:
                    ...
                    logger.info("📊 이미지 추출 메타데이터 저장 완료")
                else:
            ...
            try:
                ...
                with tempfile.TemporaryDirectory() as temp_dir:
                    ...
                    success = converter.convert_pdf_to_markdown(
                        str(file_path), str(temp_output_path), progress_callback=progress_callback
                    )
                    ...
                    if success and temp_output_path.exists():
                        ...
                        if self.image_extraction_metadata:
                            document.metadata.update({"image_extraction_metadata": self.image_extraction_metadata})
        ...
        try:
            ...
            try:
                ...
                if documents and len(documents) > 0:
                    ...
                    for doc in documents:
                        ...
                        # 이미지 추출 메타데이터가 있으면 추가
                        if self.image_extraction_metadata:
                            doc.metadata.update({"image_extraction_metadata": self.image_extraction_metadata})
            ...
            try:
                ...
                with tempfile.TemporaryDirectory() as temp_dir:
                    ...
                    success = converter.convert_pdf_to_markdown(str(file_path), str(temp_output_path))
            ...
            for doc in documents:
                doc.metadata.update(
                    {"file_name": Path(file_path).name, "file_type": ".pdf", "processing_method": "pypdf_loader"}
                )
