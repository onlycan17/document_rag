...
class DocumentSplitter:
    ...
    def split_documents(
        self, documents: List[Document], strategy: str = "auto", preserve_structure: bool = True
    ) -> List[Document]:
        """
        문서 리스트 분할
        ...
        try:
            ...
            for doc in documents:
                ...
                # 문서 분할
                chunks = self._split_single_document(doc, chosen_strategy, preserve_structure)
    ...
    def _choose_strategy(self, text: str) -> str:
        ...
        try:
            ...
            # 구조화된 텍스트 검사
            has_structure = bool(
                re.search(r"^제?\s*\d+\s*[장절편부]", text, re.MULTILINE)
    ...
    def _split_single_document(self, document: Document, strategy: str, preserve_structure: bool) -> List[Document]:
        """단일 문서 분할"""
        try:
            ...
            for section in sections:
                if len(section) <= self.max_chunk_size:
                    ...
                    if len(section.strip()) >= self.min_chunk_size:
                        chunks.append(Document(page_content=section.strip(), metadata=metadata.copy()))
                else:
                    # 섹션이 크면 문단으로 추가 분할
                ...
                if len(content) > self.max_chunk_size:
                    ...
                    for sub_content in sub_chunks:
                        if len(sub_content.strip()) >= self.min_chunk_size:
                            chunks.append(Document(page_content=sub_content.strip(), metadata=doc_metadata.copy()))
                else:
                    if len(content.strip()) >= self.min_chunk_size:
                        chunks.append(Document(page_content=content.strip(), metadata=doc_metadata))
    ...
    def _korean_split(self, text: str, metadata: Dict) -> List[Document]:
        ...
        try:
            ...
            for content in korean_chunks:
                if len(content.strip()) >= self.min_chunk_size:
                    chunks.append(Document(page_content=content.strip(), metadata=metadata.copy()))
    ...
    def _merge_small_chunks(self, chunks: List[Document]) -> List[Document]:
        ...
        try:
            ...
            while i < len(chunks):
                ...
                if len(current.page_content) < self.merge_threshold and i < len(chunks) - 1:
                ...
                if len(content) > self.max_chunk_size:
                    ...
                    for sub_content in sub_chunks:
                        if len(sub_content.strip()) >= self.min_chunk_size:
                            ...
                            valid_chunks.append(Document(page_content=sub_content.strip(), metadata=sub_metadata))
                    continue
