...
class SemanticChunker:
    ...
    def __init__(self, min_chunk_size: int = 300, max_chunk_size: int = 1500, similarity_threshold: float = 0.3):
        ...
        self.topic_transition_keywords = {
            '시간적_전환': [
                '그러나', '한편', '이후', '다음으로', '그 후', '뒤이어', '연이어',
                '시기', '시대', '년도', '세기', '기원전', '기원후', '당시',
                '이전', '이후', '동시에', '같은 시기', '그 시절'
            ],
            '공간적_전환': [
                '반면', '다른 지역', '인근', '주변', '동쪽', '서쪽', '남쪽', '북쪽',
                '지역', '위치', '장소', '구역', '지점', '현장', '터', '지구'
            ],
            '논리적_전환': [
                '따라서', '그러므로', '결과적으로', '결론적으로', '요약하면',
                '즉', '다시 말해', '예를 들어', '구체적으로', '특히', '특별히'
            ],
            '대조적_전환': [
                '그러나', '하지만', '반면에', '이와 달리', '이에 반해', '대조적으로',
                '반대로', '한편으로는', '다른 한편으로는'
            ],
            '구조적_전환': [
                '첫째', '둘째', '셋째', '마지막으로', '다음', '또한', '그리고',
                '제1장', '제2장', '제1절', '제2절', '가.', '나.', '1)', '2)'
            ]
        }
    ...
    def _detect_topic_transitions(self, paragraphs: List[str], paragraph_keywords: List[List[Tuple[str, float]]]) -> List[int]:
        """주제 전환점 감지"""
        transition_points = []
    ...
    def _adjust_boundaries_by_similarity(self, paragraphs: List[str], paragraph_keywords: List[List[Tuple[str, float]]], initial_boundaries: List[int]) -> List[int]:
    ...
    def _create_adaptive_chunks(self, paragraphs: List[str], boundaries: List[int], paragraph_keywords: List[List[Tuple[str, float]]]) -> List[Tuple[str, List[Tuple[str, float]]]]:
