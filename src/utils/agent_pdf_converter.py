...
class AgentBasedPDFConverter:
    """
    외부 API LLM 에이전트들을 활용한 고품질 PDF-to-Markdown 변환기
    """
    ...
    def __init__(
        ...
        provider, model = self._resolve_runtime_llm(llm_provider, llm_model)

        # 에이전트 초기화(선택된 제공자/모델로 동기화)
        self.context_agent = ContextConnectorAgent(provider=provider, model_name=model)
        self.structure_agent = StructureParserAgent(provider=provider, model_name=model)

        if enable_quality_validation:
            self.quality_agent = QualityValidatorAgent(provider=provider, model_name=model)
    ...
    def _resolve_runtime_llm(self, llm_provider: str | None, llm_model: str | None) -> tuple[str, str | None]:
        """화면 선택값(Streamlit 세션) → 인자 → 설정 순으로 제공자/모델 결정"""
        ...
        try:
            ...
            if quality_report:
                ...
                comparison["improvement_percentage"] = (
                    ...
                    / max(comparison["fallback_quality_score"], 0.1)
                    * 100
                )
