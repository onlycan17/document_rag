...
출력(교정된 텍스트만):"""
        ...
        try:
            ...
            logger.info(f"🔄 청크 {chunk_num}/{total_chunks} 처리 시작 (모델: {self.model_name})")
            response = self._call_llm(prompt, temperature=0.1, max_tokens=settings.max_tokens)
            if response.strip():
    ...
    def _merge_chunks(self, chunks: List[str]) -> str:
        ...
        for i in range(1, len(chunks)):
            ...
            if (
                ...
                and not prev_end.endswith("다.")
                and not curr_start.startswith("#")
            ):  # 헤더가 아닌 경우
                # 자연스러운 연결
