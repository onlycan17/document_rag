...
class ParallelProcessor:
    ...
    def start_processing(self) -> ProcessingStats:
        ...
        try:
            ...
            logger.info(f"📊 성공: {self.stats.completed_tasks}/{self.stats.total_tasks}개 "
    ...
    def _process_single_file(self, task_id: str, task: ProcessingTask) -> bool:
        ...
        try:
            ...
            for iteration in range(1, task.max_iterations + 1):
                ...
                try:
                    ...
                    if quality_result['passed']:
                        ...
                        logger.info(f"✅ {Path(task.input_path).name} 처리 완료: {task.quality_score:.1f}점 "
                                   f"({iteration}회 반복)")
                        return True
                    ...
                    if iteration < task.max_iterations:
                        ...
                        logger.info(f"🔄 {Path(task.input_path).name} 재처리 필요: {task.quality_score:.1f}점 "
            ...
            logger.warning(f"⚠️ {Path(task.input_path).name} 최대 반복 도달: {task.quality_score:.1f}점 "
