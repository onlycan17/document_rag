        ...
        def connect_text_test(lines: list) -> str:
            ...
            while i < len(lines):
                ...
                if i < len(lines) - 1:
                    ...
                    if (is_incomplete_sentence_test(current_line) and 
                        next_line and not next_line.startswith('그러나')):
...
def test_topic_transition_detection():
    ...
    try:
        ...
        false_positives = sum(1 for t in transitions if t not in expected_transitions)
...
def test_end_to_end_workflow():
    ...
    try:
        ...
        for i, chunk in enumerate(chunks):
            ...
            # 품질 기준 확인
            chunk_quality = (
                len(chunk['text']) >= 100 and  # 최소 크기
                semantic_info['topic_coherence'] > 0.2 and  # 적절한 일관성
                len(semantic_info['keywords']) > 0  # 키워드 존재
            )
