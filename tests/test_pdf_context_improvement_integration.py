...
def test_cross_page_text_connection():
    ...
    try:
        ...
        # 연결 품질 확인
        is_well_connected = (
            "토기와 철기" in connected_text  # '토'와 '기'가 연결되었는지
            and "연대측정" in connected_text  # '연'과 '대측정'이 연결되었는지
        )
...
def test_semantic_chunking_quality():
    ...
    try:
        ...
        chunker = SemanticChunker(min_chunk_size=300, max_chunk_size=1200, similarity_threshold=0.3)
        ...
        topic_transitions_detected = any(
            "제1장" in chunk["text"]
            and "제2장" not in chunk["text"]
            or "제2장" in chunk["text"]
            and "제1장" not in chunk["text"]
            for chunk in chunks
        )
