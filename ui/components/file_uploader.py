...
def _handle_file_upload(uploaded_files: List[Any], safe_get_vector_db, preprocessing_settings: Dict[str, Any]) -> None:
    """파일 업로드 처리를 담당합니다."""
...
def _reinitialize_document_loader(preprocessing_settings: Dict[str, Any]) -> None:
    ...
    # 설정 변경 감지
    settings_changed = (
        preprocessing_settings.get("use_agent_mode", False) != current_loader.use_agent_preprocessing
        or preprocessing_settings.get("enable_postprocessing", False)
        != getattr(current_loader, "enable_postprocessing", False)
        or preprocessing_settings.get("use_intelligent_extraction", False)
        != getattr(current_loader, "use_intelligent_image_extraction", False)
        or preprocessing_settings.get("selected_preprocessing_model", "local")
        != getattr(current_loader, "preprocessing_model", "local")
    )
...
def _process_uploaded_files(
    uploaded_files: List[Any], safe_get_vector_db, preprocessing_settings: Dict[str, Any]
) -> None:
    """업로드된 파일들을 처리합니다."""
...
def _process_single_file(
    ...
    processing_result = _execute_document_processing(
        uploaded_file, temp_path, update_progress, safe_get_vector_db, preprocessing_settings
    )
...
def _execute_document_processing(
    uploaded_file: Any, temp_path: str, update_progress, safe_get_vector_db, preprocessing_settings: Dict[str, Any]
) -> Dict[str, Any]:
    """문서 처리를 실행합니다."""
    ...
    try:
        ...
        image_extraction_result = _check_image_extraction_result(
            uploaded_file, preprocessing_settings.get("use_intelligent_extraction", False)
        )
        ...
        if documents or image_extraction_result["success"]:
            ...
            processing_time = time.time() - start_time
            return {"success": False, "chunks": 0, "processing_time": processing_time, "error": "문서를 로드할 수 없음"}

    except Exception as e:
        processing_time = time.time() - start_time
        return {"success": False, "chunks": 0, "processing_time": processing_time, "error": str(e)}
...
def _update_document_loader_settings(preprocessing_settings: Dict[str, Any]) -> None:
    ...
    # 설정 확인 및 업데이트
    if (
        getattr(current_loader, "use_ocr", True) != preprocessing_settings.get("use_ocr", True)
        or getattr(current_loader, "use_agent_preprocessing", False)
        != preprocessing_settings.get("use_agent_mode", False)
        or getattr(current_loader, "use_intelligent_image_extraction", False)
...
def _check_image_extraction_result(uploaded_file: Any, use_intelligent_extraction: bool) -> Dict[str, Any]:
    ...
    if images_dir.exists():
        extracted_images = (
            list(images_dir.glob("*.jpeg")) + list(images_dir.glob("*.jpg")) + list(images_dir.glob("*.png"))
        )
...
def _display_quality_verification(result: Dict[str, Any]) -> None:
    ...
    if result["chunks"] > 0:
        if result["chunks"] == result.get("saved_chunks", 0):
            ...
            st.warning(
                f"⚠️ 품질 검증: 로드된 청크({result['chunks']}개) vs 저장된 청크({result.get('saved_chunks', 0)}개)"
            )
...
def _display_processing_summary(
    ...
    with stat_col2:
        st.metric(
            "처리 성공", f"{processed_files}개", delta=f"{failed_files}개 실패" if failed_files > 0 else "모두 성공"
        )
