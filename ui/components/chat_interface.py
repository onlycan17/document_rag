    ...
    try:
        ...
        if stream_gen is not None:
            ...
            with st.spinner("답변을 생성하는 중..."):
                sync_result = rag_chain.invoke(prompt, session_id=st.session_state.langsmith_session_id)
...
def _get_stream_generator(rag_chain: RAGChain, prompt: str):
    if not hasattr(rag_chain, "stream_query"):
        return None
    try:
        return rag_chain.stream_query(prompt, session_id=st.session_state.langsmith_session_id)
    except Exception:
        return None
